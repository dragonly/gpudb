 ...
 */
    ...
    if(dataPos == MEM || dataPos == MMAP || dataPos == PINNED){
#ifdef NOHAS_GMM
        CUDA_SAFE_CALL_NO_SYNC(cudaMallocEx((void**)&gpu_dim, primaryKeySize, FLAG_COW));
#else
        ...
        CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(gpu_dim, jNode->rightTable->content[jNode->rightKeyIndex], primaryKeySize, (enum cudaMemcpyKind)(cudaMemcpyHostToDevice)));
 ...
 */
    ...
    if(dataPos == MEM || dataPos == MMAP || dataPos == PINNED){
#ifdef NOHAS_GMM
        CUDA_SAFE_CALL_NO_SYNC(cudaMallocEx((void**)&gpu_fact, foreignKeySize, FLAG_COW));
#else
        ...
        gettimeofday(&t, NULL);
        //printf("[gvm] %lf intercepted diskIO\n", t.tv_sec + t.tv_usec / 1000000.0);
        CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(gpu_fact, jNode->leftTable->content[jNode->leftKeyIndex], foreignKeySize, (enum cudaMemcpyKind)(cudaMemcpyHostToDevice)));
    ...
    for(int i=0; i<res->totalAttr; i++){
        ...
        if(leftRight == 0){
            if(format == UNCOMPRESSED){
                ...
                if(dataPos == MEM || dataPos == MMAP || dataPos == PINNED){
#ifdef NOHAS_GMM
                    CUDA_SAFE_CALL_NO_SYNC(cudaMallocEx((void **)&gpu_fact, colSize, FLAG_COW));
#else
