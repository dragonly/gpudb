...
__device__ static inline void ComparatorInt(int &keyA, int &valA, int &keyB, int &valB, int dir) {
  int t;
...
__device__ static inline void ComparatorFloat(float &keyA, int &valA, float &keyB, int &valB, int dir) {
  float t1;
  int t2;
...
__device__ static inline void Comparator(char *keyA, int &valA, char *keyB, int &valB, int keySize, int dir) {
  int t;
  char buf[32];
...
extern "C" __global__ void sort_key_string(char *key, int tupleNum, int keySize, char *result, int *pos, int dir) {
  ...
  for (int size = 2; size < tupleNum && size < SHARED_SIZE_LIMIT; size <<= 1) {
    ...
    for (int stride = size / 2; stride > 0; stride >>= 1) {
      ...
      Comparator(bufKey + pos * keySize, bufVal[pos + 0], bufKey + (pos + stride) * keySize, bufVal[pos + stride],
                 keySize, ddd);
    }
  }
...
extern "C" __global__ void sort_key_int(int *key, int tupleNum, int *result, int *pos, int dir) {
  ...
  for (int size = 2; size < tupleNum && size < SHARED_SIZE_LIMIT; size <<= 1) {
    ...
    for (int stride = size / 2; stride > 0; stride >>= 1) {
      __syncthreads();
      int pos = 2 * threadIdx.x - (threadIdx.x & (stride - 1));
      ComparatorInt(bufKey[pos + 0], bufVal[pos + 0], bufKey[pos + stride], bufVal[pos + stride], ddd);
    }
  }

  {
    for (int stride = blockDim.x; stride > 0; stride >>= 1) {
      __syncthreads();
      int pos = 2 * threadIdx.x - (threadIdx.x & (stride - 1));
      ComparatorInt(bufKey[pos + 0], bufVal[pos + 0], bufKey[pos + stride], bufVal[pos + stride], dir);
    }
  }
...
extern "C" __global__ void sort_key_float(float *key, int tupleNum, float *result, int *pos, int dir) {
  ...
  for (int size = 2; size < tupleNum && size < SHARED_SIZE_LIMIT; size <<= 1) {
    ...
    for (int stride = size / 2; stride > 0; stride >>= 1) {
      __syncthreads();
      int pos = 2 * threadIdx.x - (threadIdx.x & (stride - 1));
      ComparatorFloat(bufKey[pos + 0], bufVal[pos + 0], bufKey[pos + stride], bufVal[pos + stride], ddd);
    }
  }

  {
    for (int stride = blockDim.x; stride > 0; stride >>= 1) {
      __syncthreads();
      int pos = 2 * threadIdx.x - (threadIdx.x & (stride - 1));
      ComparatorFloat(bufKey[pos + 0], bufVal[pos + 0], bufKey[pos + stride], bufVal[pos + stride], dir);
    }
  }
...
#ifdef HAS_GMM
  CUDA_SAFE_CALL_NO_SYNC(cudaMallocEx((void **)&gpuContent, sizeof(char *) * res->totalAttr, FLAG_PTARRAY));
#else
