...
struct tableNode *groupBy(struct groupByNode *gb, struct statistic *pp) {
  ...
  for (int i = 0; i < gb->table->totalAttr; i++) {
    ...
    if (gb->table->dataPos[i] == MEM) {
      ...
      CUDA_SAFE_CALL_NO_SYNC(
          cudaMemcpy(column[i], gb->table->content[i], attrSize * gb->table->tupleNum, cudaMemcpyHostToDevice));

      CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(&gpuContent[i], &column[i], sizeof(char *), cudaMemcpyHostToDevice));
    } else {
      CUDA_SAFE_CALL_NO_SYNC(
          cudaMemcpy(&gpuContent[i], &gb->table->content[i], sizeof(char *), cudaMemcpyHostToDevice));
    }
  }
  ...
  if (gbConstant != 1) {
    ...
    CUDA_SAFE_CALL_NO_SYNC(
        cudaMemcpy(gpuGbType, gb->groupByType, sizeof(int) * gb->groupByColNum, cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL_NO_SYNC(cudaMalloc((void **)&gpuGbSize, sizeof(int) * gb->groupByColNum));
    CUDA_SAFE_CALL_NO_SYNC(
        cudaMemcpy(gpuGbSize, gb->groupBySize, sizeof(int) * gb->groupByColNum, cudaMemcpyHostToDevice));
    ...
    CUDA_SAFE_CALL_NO_SYNC(
        cudaMemcpy(gpuGbIndex, gb->groupByIndex, sizeof(int) * gb->groupByColNum, cudaMemcpyHostToDevice));
  ...
  CUDA_SAFE_CALL_NO_SYNC(
      cudaMemcpy(gpuMathOffset, mathExpOffset, sizeof(int) * res->totalAttr, cudaMemcpyHostToDevice));
  ...
  for (int i = 0; i < res->totalAttr; i++) {
    ...
    if (gb->gbExp[i].exp.opNum == 2) {
      CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(&(gpuMathExp[offset]), (struct mathExp *)gb->gbExp[i].exp.exp,
                                        2 * sizeof(struct mathExp), cudaMemcpyHostToDevice));
    } else {
      CUDA_SAFE_CALL_NO_SYNC(
          cudaMemcpy(&(gpuMathExp[offset]), &(gb->gbExp[i].exp), sizeof(struct mathExp), cudaMemcpyHostToDevice));
    }
  }
