    ...
    if(1){
...
#ifdef NOHAS_GMM
            CUDA_SAFE_CALL_NO_SYNC(cudaMallocEx((void **) &column[whereIndex], sn->tn->attrTotalSize[index], FLAG_COW));
#else
        ...
        if(format == UNCOMPRESSED){
            if(sn->tn->dataPos[index] == MEM || sn->tn->dataPos[index] == MMAP || sn->tn->dataPos[index] == PINNED){
                ...
                CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(column[whereIndex], sn->tn->content[index], sn->tn->attrTotalSize[index], (enum cudaMemcpyKind)(cudaMemcpyHostToDevice)));
 ...
 */
    ...
    for(int i=0;i<attrNum;i++){
        ...
        if(pos != -1){
            ...
            if(sn->tn->dataPos[index] == MEM || sn->tn->dataPos[index] == MMAP || sn->tn->dataPos[index] == PINNED)
#ifdef NOHAS_GMM
                CUDA_SAFE_CALL_NO_SYNC(cudaMallocEx((void **) &scanCol[i] , sn->tn->attrTotalSize[index], FLAG_COW));
#else
            ...
            if(sn->tn->dataFormat[index] != DICT){
                if(sn->tn->dataPos[index] == MEM || sn->tn->dataPos[index] == MMAP || sn->tn->dataPos[index] == PINNED) {
					...
					gettimeofday(&t, NULL);
					//printf("[gvm] %lf intercepted diskIO\n", t.tv_sec + t.tv_usec / 1000000.0);
                    CUDA_SAFE_CALL_NO_SYNC(cudaMemcpy(scanCol[i], sn->tn->content[index], sn->tn->attrTotalSize[index], (enum cudaMemcpyKind)(cudaMemcpyHostToDevice)));
                } else {
